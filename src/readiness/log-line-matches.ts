...
export const logLineMatchesProbe: ReadinessProbe<typeof LogLineMatchesProbeConfigSchema> = {
  ...
  async evaluate(ctx: ReadinessContext, config: LogLineMatchesProbeConfig): Promise<ReadinessOutcome> {
    ...
    const tail = await readTailFromOffset(handle.logFile, handle.logStartOffset, config.maxBytes);
    if (!tail) {
      return { ok: false, detail: `log file ${handle.logFile} not found yet` };
    }
    if (tail.text.length === 0) {
      return { ok: false, detail: `log file ${handle.logFile} has no new bytes since logStartOffset ${handle.logStartOffset}` };
    }
